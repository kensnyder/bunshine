...
import { type EtagHashCalculator, etags } from './etags';
...
describe('etags middleware', () => {
  ...
  it('If-Match failing precondition returns 412, passing proceeds', async () => {
    ...
    const fail = await fetch(server.url, {
      method: 'PUT',
    ...
    const pass = await fetch(server.url + '/check', {
      method: 'PUT',
  ...
  it('wildcard handling: If-None-Match * matches; If-Match * passes', async () => {
    app.get('/', c => c.text('Hello world'));
    expect(
      (await fetch(server.url, { headers: { 'If-None-Match': '*' } })).status
    ).toBe(304);
    app.put('/put', c => c.text('ok'));
    expect(
      (
        await fetch(server.url + '/put', {
          method: 'PUT',
          headers: { 'If-Match': '*' },
        })
      ).status
  ...
  it('HEAD: does not compute but honors existing ETag for conditionals', async () => {
    ...
    app.head(
      '/h',
      _c => new Response(null, { headers: { ETag: helloWorldEtag } })
    );
    resp = await fetch(server.url + '/h', {
      method: 'HEAD',
      headers: { 'If-None-Match': helloWorldEtag },
    });
    expect(resp.status).toBe(304);
    expect(resp.headers.get('ETag')).toBe(helloWorldEtag);
  });
  ...
  it('normalizes Etag header casing and does not overwrite existing when overwrite=false', async () => {
    ...
    app.get(
      '/',
      _ => new Response('Hello world', { headers: { Etag: helloWorldEtag } })
    );
    const resp = await fetch(server.url);
    expect(resp.headers.get('ETag')).toBe(helloWorldEtag);
  ...
  it('overwrite=true recalculates and replaces existing ETag; sets Vary', async () => {
    const calc: EtagHashCalculator = async (_c, _r) => ({
      buffer: new TextEncoder().encode('abc'),
      hash: 'deadbeef',
    });
    ...
    app.get(
      '/',
      _ => new Response('Hello world', { headers: { ETag: '"old"' } })
    );
    const resp = await fetch(server.url);
  ...
  it('does not generate for 204/empty body, 206, no-store, DELETE, and 0 Content-Length', async () => {
    ...
    resp = await fetch(server.url + '/s206', {
    ...
    app.get(
      '/nostore',
      _ => new Response('x', { headers: { 'Cache-Control': 'no-store' } })
    );
    ...
    app.get(
      '/zero',
      _ => new Response('', { headers: { 'Content-Length': '0' } })
    );
