...
// Inlined from type-fest to keep routeshine zero-dependency
type TypedArray =
  | Int8Array
  | Uint8Array
  | Uint8ClampedArray
...
export function etags({
  ...
  const exceptWhenResult = withTryCatch({
    label: 'Routeshine etags middleware exceptWhen error',
    defaultReturn: false,
    func: exceptWhen,
  });
  ...
  const safeCalculator = withTryCatch({
    label: 'Routeshine etags middleware calculator error',
  ...
  return async (context: Context, next: NextFunction) => {
    ...
    if (computedBuffer) {
      ...
      const statusText = (resp as any).statusText;
      resp.headers.delete('Content-Length');
      // @ts-expect-error statusText may not be in all typings
